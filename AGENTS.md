...
11. The Web client wraps the exact DSH `0.1.2-alpha.1` `sendSession`/`beginSubmission` seam because no public atomic pre-submit middleware exists. Any DSH upgrade must re-audit the conversation and session-lifecycle seams, update the README compatibility statement, and exercise a real packed Web boot.
